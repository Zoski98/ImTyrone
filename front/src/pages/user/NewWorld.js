...
import profile from '../../media/Illustrations/user-profile.png'
...
function NewWorld() {
  ...
  useEffect(() => {
    async function test() {
      const res = await axios.get('http://127.0.0.1:8000/api/world/user/posts');
  ...
  useEffect(() => {
    async function testy() {
      const res = await axios.get('http://127.0.0.1:8000/api/world/last-posts');
      if (res.data.status === 200) {
        ...
        <div class="serious-bot">
          <div class="serious-bot-left">
            <div class="serious-bot-left-header">
              <h2 class="serious-section-name">WORLD</h2>
